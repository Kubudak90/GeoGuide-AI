            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
                ...
                <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-emerald-50/50 dark:bg-emerald-900/10">
                    <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                        <span className="text-red-500">❤️</span> {t('my_favorites')}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <div className="overflow-y-auto p-4 flex-1">
                    {favorites.length === 0 ? (
                        <div className="text-center py-10 text-gray-400 dark:text-gray-500">
                            ...
                            <p>{t('no_favorites')}</p>
                            <p className="text-sm mt-1">{t('start_exploring')}</p>
                        </div>
                    ) : (
                            ...
                            {favorites.map((place, index) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onSelect(place)}>
                                        <h3 className="font-semibold text-gray-800 dark:text-gray-100 truncate">{place.name}</h3>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{place.category || t('saved_place')}</p>
                                    </div>
